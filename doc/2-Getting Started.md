...
调用成功会回调`onSuccess`方法，可通过类似`resp.parm1`和`resp.parm2`获取相关返回参数，本文提到的“成功回调返回参数”即指这些提取出来的参数，如`parm1`和`param2`，后文不再赘述。  
`FSOpen.init`执行成功即代表API运行环境初始化成功，无需处理返回结果，当然开发者还是可以在`onSuccess`回调中添加自己的代码，如数据统计、表单提交等。   
后文中如接口执行成功无需处理返回结果，将省略成功返回说明。  

失败回调返回参数  

调用失败会回调`onFail`方法，可通过`error.errorCode`和`error.errorMessage`获取错误描述，后文不再赘述。  
...
后文中接口调用失败如无特殊情况，将省略失败返回说明，开发者可自行查阅[错误码汇总表]()。  
*TODO：错误码锚点*
...
<html lang="zh-CN">
  <head>
    ...
    <script src="http://www.fxiaoke.com/open/jsapi/1.0.0/fsopen.min.js"></script>
  </head>
